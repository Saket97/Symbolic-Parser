def discover_nt():
	...
	for i in range(len(grammar)):
		if grammar[i][0] not in nt:
			nt.append(grammar[i][0])
	return nt
def process_parse(nt):
	rules = {}
	for key in ptable[nt]:
		if ptable[nt][key] in rules:
			rules[ptable[nt][key]].append(key)
		else:
			rules.update({ptable[nt][key]:[key]})
	return rules
def discover_tokens():
	terminals = []
	for i in range(len(grammar)):
	...
	for i in range(len(terminals)):
		tdict.update({"%s"%terminals[i]:i})
	return terminals,tdict

def write_file(output_file):
	output_file.write("int map(char*s){\n")
	for t in range(len(terminals)):
		if t != 0:
			output_file.write("	else\n")
		output_file.write('	if (strcmp(s,"%s") == 0)\n\treturn %d;\n'%(terminals[t], tdict[terminals[t]]))
	output_file.write("}\n")
	for nt in non_terminals:
		output_file.write("void %s();\n"%nt)
	output_file.write("""void consume_token(int val)
{
	if (val == map(s[pos]))
	pos += 1;
	else
	...
	for nt in non_terminals:
		output_file.write("void %s(){\n"%nt)
		write_dict = process_parse(nt)
		output_file.write("""	//printf("%s %%s\\n",s[pos]);\n"""%nt)
		output_file.write("	switch(map(s[pos])){\n")
		for key in write_dict:
			if key == 0:
				output_file.write('		default:\n\t\tprintf("Error! pos=%%d %s\\n",pos);\n\t\texit(1);\n'%nt)
			else:
				for t in write_dict[key]:
					output_file.write('		case %d:\n'%tdict[t])
				for element in range(1,len(grammar[key-1])):
					if grammar[key-1][element] == "eps":
						continue
					if grammar[key-1][element] in non_terminals:
						output_file.write("		%s();\n"%grammar[key-1][element])
					else:
						output_file.write('		consume_token(map("%s"));\n'%grammar[key-1][element])
...
ptable['LD_extra']['[']= 59
non_terminals = discover_nt()
terminals, tdict = discover_tokens()
output_file = open("output.c",'w')
output_file.write("#include <stdio.h>\n#include <stdlib.h>\n#include <string.h>\nchar s[30][10];\nint pos;\n")
write_file(output_file)
output_file.write("""void pre_process(char s[][10], int* arr, int n)
{
	for (int i = 0; i < n; ++i)
	arr[i] = map(s[i]);
}
