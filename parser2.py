...
# if fuel = 0, then call this function to decide how that non-terminal should be expanded i.e. consider the rule with minimum non-terminals
def find_with_least_t(rules): #rules contain the rule numbers which can be used to expand the given non-terminals.
	rules_t = []
	assert (len(rules) > 0)
	minimum = [10,rules[0]] # minimum[0] is the number of non-terminals and minimum[1] is the rule number
	onlyeps = True
##### Finding the minimum number of non-terminals among the possible rules #####################
	for i in rules:
		r = 0
		for j in range(1,len(grammar[i])):
			if grammar[i][j] not in terminals:
				r += 1
			if grammar[i][j] != "eps":
				onlyeps = False
		print "rule: %d  n_nt:%d min[0]:%d"%(i,r,minimum[0])
		if r < minimum[0]:
			minimum[0] = r
			minimum[1] = i
##### Forming rules_t which contain the rule numbers which have minimum[0] number of non-terminals #############
	for i in rules:
		r = 0
		for j in range(1,len(grammar[i])):
			if grammar[i][j] not in terminals:
		...
		if r == minimum[0]:
			rules_t.append(i)
#################################################################################################################
	minimum[1] = rules_t[random.randint(0,len(rules_t)-1)]
	return minimum

##### This function returns which rule should be used to expand the non-terminal based on whether fuel is 0 or not #########
def next_rule(nt):
	print "nt:",nt
	rules = []
	global fuel
	# rules contain the list of rule numbers whose LHS contain that non-terminal.
	###########################################################################
	for key in ptable[nt]:
		if ptable[nt][key] != 0 and ptable[nt][key] not in rules:
			rules.append(ptable[nt][key])
	print "rules:",rules
	############################################################################
	if fuel == 0:
		print "fuel extinguished..."
		rule_t = find_with_least_t(rules) # if fuel is 0 then chooose rule with minimum non-terminals.
		return rule_t[1]
	# if fuel is not 0, then randomly select among the possible rules.
	n = random.randint(0,len(rules)-1)
	print "randomly generated:",n
	fuel -= 1
	return rules[n]
...
def parser():
	...
	while len(parse) != 0:
		...
		if ontop in terminals:
			string.append(ontop)
			print "consuming token %s"%ontop
			# parse.pop()
		else:
			ruleno = next_rule(ontop)
			print "nxt rule:",ruleno
			parse = expand(ruleno, parse, ontop)
			if parse == -1:
